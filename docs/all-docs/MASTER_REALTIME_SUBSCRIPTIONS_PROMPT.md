...
- **Reconnection Handling**: Handle connection errors gracefully with retry logic
...
```typescript
// Real example from useCheckoutRealtime
.subscribe((status) => {
  if (status === 'SUBSCRIBED') {
    ...
    setTimeout(() => {
      try {
        ...
        logger.warn('Failed to retry subscription:', retryErr)
      }
    }, 2000)
...
- [ ] Handle CHANNEL_ERROR status
- [ ] Handle TIMED_OUT status with retry
- [ ] Handle CLOSED status
- [ ] Check for table existence errors
- [ ] Graceful degradation (app works without realtime)
- [ ] Log errors for debugging
- [ ] Don't show errors to users for background failures
