...
function checkFileForFeatureFlag(filePath, flag) {
  ...
  try {
    ...
    // Check for flag name (enable_product_customization, enable_reservations, etc.)
    const flagPattern = new RegExp(flag.replace(/_/g, '[_\\.]'), 'i');
    const foundFlag = flagPattern.test(content);
    
    // Check for prop name (enableCustomization, enableReservations, etc.)
    // Convert enable_product_customization to enableCustomization
    // Handle special cases: enable_product_customization -> enableCustomization (simplified)
    let propName = flag.replace(/^enable_/, '');
    
    // Special mapping for known simplified prop names
    const propMapping = {
      'product_customization': 'enableCustomization',
      'menu_filters': 'enableMenuFilters',
      'order_tracking': 'enableOrderTracking',
      'order_feedback': 'enableOrderFeedback',
      'marketing_optins': 'enableMarketingOptins',
      'quick_reorder': 'enableQuickReorder',
      'loyalty_program': 'enableLoyalty',
      'reservations': 'enableReservations',
    };
    
    // Check for mapped prop name first
    let camelCaseProp = propMapping[propName];
    
    // If no mapping, convert to camelCase
    if (!camelCaseProp) {
      const camelCase = propName
        .replace(/_([a-z])/g, (_, letter) => letter.toUpperCase());
      camelCaseProp = `enable${camelCase.charAt(0).toUpperCase()}${camelCase.slice(1)}`;
    }
    
    const propPattern = new RegExp(camelCaseProp, 'i');
    const foundProp = propPattern.test(content);
    
    const found = foundFlag || foundProp;
...
function checkMigrationFile() {
  ...
  try {
    ...
    return { exists: true, hasAllFlags };
  } catch (error) {
