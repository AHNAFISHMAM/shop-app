 ...
 * ```
 */
export interface CalendarProps extends Omit<DayPickerProps, 'classNames'> {
  /**
...
const DEFAULT_CLASS_NAMES = {
  ...
  head_cell: "w-full text-center text-xs sm:text-sm font-semibold text-[var(--text-secondary)] uppercase tracking-wider py-2 box-border",
  ...
  weekday: "w-full text-center text-xs sm:text-sm font-semibold text-[var(--text-secondary)] uppercase tracking-wider py-2 box-border",
  day_button:
  ...
  outside: "text-[var(--text-secondary)] data-selected:bg-[var(--accent)]/50 data-selected:text-[var(--text-secondary)]",
  hidden: "invisible",
  week_number: "min-h-[44px] min-w-[44px] h-11 w-11 p-0 text-xs font-medium text-[var(--text-secondary)]",
} as const
    ...
    return Object.keys(DEFAULT_CLASS_NAMES).reduce(
      (acc, key) => {
        ...
        return {
          ...acc,
          [key]: customClassName
            ? cn(defaultClassName, customClassName)
            : defaultClassName,
        }
      },
