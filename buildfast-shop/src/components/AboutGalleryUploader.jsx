 ...
 */
function AboutGalleryUploader({ onUploadSuccess }) {
   ...
   */
  const handleUpload = async (slot, imageType) => {
    const slotKey = `${slot}_${imageType}`
    ...
    try {
      ...
      document.getElementById(`about-upload-${slotKey}`).value = ''
  ...
  const renderUploadSlot = (slotKey, label, imageType) => {
          ...
          {cardPairs.map(({ slot, label, animation, description }) => (
            ...
            >
              <div className="space-y-4">
                ...
                {renderUploadSlot(`${slot}_default`, 'Default Image', 'default')}

                {/* Hover Image Upload */}
                {renderUploadSlot(`${slot}_hover`, 'Hover Image', 'hover')}
              </div>
            </div>
          ))}
        </div>
...
AboutGalleryUploader.propTypes = {
