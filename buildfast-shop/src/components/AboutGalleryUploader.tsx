      ...
      <div className="space-y-2">
        ...
        {hasFile && (
          <p className="text-xs" style={{ color: 'var(--accent)' }}>
            {hasFile.name.length > 25 ? hasFile.name.substring(0, 25) + '...' : hasFile.name}
          </p>
        )}
      ...
      <div className="space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
            Upload both default and hover images for each card. Each card has a unique animation
            effect. (JPG, PNG, or WebP, max 5MB each)
          </p>
        </div>
          ...
          {cardPairs.map(({ slot, label, animation, description }) => (
            ...
            >
              <div className="space-y-4">
                ...
                {renderUploadSlot(
                  `${slot}_default` as keyof SelectedFiles,
                  'Default Image',
                  'default'
                )}
        ...
        <p className="text-xs text-center" style={{ color: 'var(--text-muted)' }}>
          Both images are required for each card. Hover effects will be visible immediately on the
          About page.
        </p>
      </div>
    </div>
