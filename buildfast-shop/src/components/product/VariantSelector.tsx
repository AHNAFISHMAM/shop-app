    ...
    <div className="space-y-4">
      {Object.entries(groupedVariants).map(([type, variants], index) => (
        ...
        >
          ...
          <div className="flex flex-wrap gap-2" role="radiogroup" aria-labelledby={`variant-${type}-label`}>
            {variants.map((variant) => {
              const selected = selectedVariants[type];
              const isSelected = typeof selected === 'object' 
              ...
              return (
                <m.button
                  ...
                  onClick={() => !isOutOfStock && handleVariantSelect(type, variant.variant_value)}
