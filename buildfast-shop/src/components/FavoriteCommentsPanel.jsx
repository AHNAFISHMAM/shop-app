...
function FavoriteCommentsPanel({ favoriteItems = [], userId }) {
  ...
  return (
    ...
    >
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.3em] text-muted">Monthly Feedback</p>
        ...
        <div 
          className="rounded-xl border border-theme px-4 py-3 text-sm text-muted"
          style={{
            backgroundColor: isLightTheme 
              ? 'rgba(0, 0, 0, 0.02)' 
        ...
        >
          <span className="block text-xs uppercase tracking-[0.25em] text-muted">This Month</span>
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        <div className="grid gap-4 md:grid-cols-[2fr,3fr] md:gap-6">
          ...
          <label className="flex flex-col gap-2">
            <span className="text-xs font-semibold uppercase tracking-[0.3em] text-muted">
              Comment <span className="text-muted lowercase">(1–300 characters)</span>
            </span>
            <div className="relative">
              <textarea
                ...
                className="w-full rounded-xl border border-theme px-3 py-2 text-sm text-[var(--text-main)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/60"
                style={{
                  backgroundColor: isLightTheme 
                    ? 'rgba(0, 0, 0, 0.02)' 
                    : 'rgba(255, 255, 255, 0.02)',
        ...
        <div className="flex flex-col gap-2">
          <span className="text-xs font-semibold uppercase tracking-[0.3em] text-muted">
            Attach Images <span className="text-muted lowercase">(optional, up to 3)</span>
          </span>
          <div className="flex flex-wrap gap-3">
            <label 
              className="flex h-20 w-20 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-theme-strong text-xs text-muted transition hover:border-[var(--accent)]/40 hover:text-[var(--accent)]"
              style={{
                backgroundColor: isLightTheme 
                  ? 'rgba(0, 0, 0, 0.02)' 
                  : 'rgba(255, 255, 255, 0.02)',
            ...
            {attachments.map((attachment, index) => (
              <div
                key={attachment.url}
                className="relative h-20 w-20 overflow-hidden rounded-xl border border-theme"
                style={{
                  backgroundColor: isLightTheme 
                    ? 'rgba(0, 0, 0, 0.02)' 
                    : 'rgba(255, 255, 255, 0.02)',
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold uppercase tracking-[0.25em] text-muted">Recent Comments</h3>
          {listLoading && (
        ...
        {!comments.length && !listLoading ? (
          <div 
            className="rounded-xl border border-theme px-4 py-6 text-center text-sm text-muted"
            style={{
              backgroundColor: isLightTheme 
                ? 'rgba(0, 0, 0, 0.02)' 
                : 'rgba(255, 255, 255, 0.02)',
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {comments.slice(0, 8).map(comment => {
              ...
              return (
                <article
                  key={comment.id}
                  className="glow-surface glow-soft flex flex-col gap-3 rounded-2xl border border-theme px-4 py-4 backdrop-blur-sm"
                  style={{
                    backgroundColor: isLightTheme 
                      ? 'rgba(0, 0, 0, 0.02)' 
                      : 'rgba(255, 255, 255, 0.02)',
                ...
                >
                  <header className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted">{formatDateTime(comment.created_at)}</p>
                    </div>
                  </header>
                  <p className="text-sm text-[var(--text-main)]">{comment.review_text}</p>
                  {comment.review_images?.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {comment.review_images.map((url, index) => (
                        <button
                          key={url}
                          type="button"
                          className="group relative h-16 w-16 overflow-hidden rounded-xl border border-theme"
