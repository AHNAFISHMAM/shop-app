...
function Root({ children }: RootProps) {
  return (
    <section
      className="relative z-0 isolate flex min-h-[calc(100vh-6rem)] w-full items-center justify-center overflow-hidden py-12"
      role="main"
    ...
    >
      ...
      <div className="w-full max-w-lg px-4 sm:px-6 lg:px-0">{children}</div>
    </section>
  )
}
...
function Card({ children }: CardProps) {
  ...
  return (
    <div
      ...
      style={{
        pointerEvents: 'auto',
        backgroundColor: isLightTheme ? 'rgba(255, 255, 255, 0.95)' : 'rgba(255, 255, 255, 0.05)',
...
function Header({ eyebrow, title, subtitle, helper }: HeaderProps) {
  return (
    <header className="space-y-3 text-center">
      {eyebrow ? (
        <p className="text-sm uppercase tracking-[0.24em] text-[var(--accent)]/80">{eyebrow}</p>
      ) : null}
      <h1 className="text-2xl font-semibold text-[var(--text-main)] sm:text-3xl">{title}</h1>
      {subtitle ? <p className="text-sm text-[var(--text-muted)]">{subtitle}</p> : null}
      {helper ? <div className="text-sm text-[var(--text-muted)]">{helper}</div> : null}
    </header>
  )
}
