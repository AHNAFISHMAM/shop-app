...
const SectionContainer = ({
  ...
  if (totalDishes === 0) {
    return (
      <div className="rounded-2xl border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] p-8 text-center" role="status" aria-live="polite">
        <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] min-h-[44px] min-w-[44px]">
          <svg className="h-8 w-8 text-[var(--status-warning-border)]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
        </div>
        <h3 className="text-xl font-semibold text-[var(--status-warning-border)] mb-2">No Special Sections Available</h3>
    ...
    >
      ...
      {sectionData.map((section) => (
        <m.div
          ...
          viewport={prefersReducedMotion ? undefined : { once: true, amount: 0.3, margin: '0px 0px -10% 0px' }}
          exit={prefersReducedMotion ? undefined : 'exit'}
        >
          <SectionCarousel
