...
const CartItemCard = memo(({
  ...
  const imageUrl = useMemo(() => {
    return getImageUrl ? getImageUrl(product) : 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=400&h=400&fit=crop';
  }, [getImageUrl, product]);

  // Stock status (if available)
  const isMenuItem = useMemo(() => {
    return product?.isMenuItem ?? (product?.category_id !== undefined && product?.is_available !== undefined);
  ...
  const hasFiniteStock = useMemo(() => stockQuantity !== null && stockQuantity !== undefined, [stockQuantity]);
  ...
  const isLowStock = useMemo(() => {
    return hasFiniteStock && stockQuantity !== null && stockQuantity !== undefined && stockQuantity > 0 && stockQuantity <= 5;
  }, [hasFiniteStock, stockQuantity]);

  const maxQuantity = useMemo(() => {
    return hasFiniteStock && !isOutOfStock && stockQuantity !== null && stockQuantity !== undefined
        ...
        <div className="cart-item-content-v2">
          <div className="cart-item-header">
            <div className="cart-item-info">
              ...
              {itemNote && (
                <p className="cart-item-note-display" role="note" aria-label={`Note: ${itemNote}`}>
                  📝 {itemNote}
                </p>
              )}
