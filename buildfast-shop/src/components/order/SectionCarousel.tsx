...
const SectionCarousel = ({
  ...
  const showContent = useMemo(() => isExpanded && isAvailable && hasDishes, [isExpanded, isAvailable, hasDishes]);
  const showEmpty = useMemo(() => isExpanded && (!isAvailable || !hasDishes), [isExpanded, isAvailable, hasDishes]);
  ...
  return (
    <section className="mb-6 animate-fade-in" aria-labelledby={`section-${sectionName.toLowerCase().replace(/\s+/g, '-')}-heading`}>
      {/* Section Header - Clean & Professional */}
      <div className="flex items-center gap-3 mb-4 pb-3 border-b border-[var(--border-subtle)]">
        <h2 id={`section-${sectionName.toLowerCase().replace(/\s+/g, '-')}-heading`} className="text-2xl md:text-3xl font-bold text-[var(--accent)]">
          {sectionName}
        </h2>
        {hasDishes && (
          <span className="px-3 py-1.5 bg-[var(--accent)]/20 text-[var(--accent)] rounded-full text-sm md:text-base font-semibold min-h-[44px] flex items-center justify-center" aria-label={`${dishes.length} items in ${sectionName}`}>
            {dishes.length}
          </span>
        )}
      </div>
      ...
      {showContent && (
        <div className="relative max-w-full overflow-hidden" role="region" aria-label={`${sectionName} products`}>
          {/* RESPONSIVE CONTAINER - Square cards (1:1) - Shows 2.5 cards desktop, 1.3 mobile */}
          <div className="overflow-x-auto scroll-smooth flex gap-4 sm:gap-6 pb-4 px-6 snap-x snap-mandatory" role="list" aria-label={`${sectionName} product list`}>
            {dishes.map((dish) => (
              <div
                key={dish.id}
