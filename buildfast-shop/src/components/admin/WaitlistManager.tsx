...
function WaitlistManager(): JSX.Element {
  ...
  const updateStatus = async (id: string, status: 'notified' | 'seated') => {
    try {
  ...
  const getWaitTime = (addedAt: string): string => {
    const minutes = Math.floor((new Date().getTime() - new Date(addedAt).getTime()) / 60000)
