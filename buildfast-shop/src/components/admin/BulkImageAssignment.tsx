...
interface BulkImageAssignmentProps {
  ...
  onBulkAssign: (assignments: Array<{ menuItemId: string | number; imageUrl: string }>) => Promise<void>;
...
export default function BulkImageAssignment({
  ...
  const [generationSummary, setGenerationSummary] = useState<GenerationSummary>({ success: 0, fail: 0, fallback: 0 });
  ...
  async function fetchUniquePhoto(menuItem: MenuItem, usedPhotoIds: Set<string>): Promise<Assignment | null> {
  ...
  async function handleAutoGenerateImages(options: { scope?: string; itemIds?: Array<string | number> } = {}): Promise<void> {
        ...
        for (const menuItem of batch) {
          ...
          try {
            ...
            if (!assignment) {
              ...
              summary.success += 1;
              setGenerationStatus(prev => ({ ...prev, [String(menuItem.id)]: assignment.duplicate ? 'duplicate' : 'success' }));
            }
  ...
  async function handleBulkUpload(): Promise<void> {
    ...
    try {
      ...
      for (const file of selectedFiles) {
        ...
        if (result[0]?.success) {
          results.push(result[0] as UploadResult);
        }
      }
                    ...
                    <p>
                      ...
                      <CustomDropdown
                        options={[
                          ...
                          ...selectionPool.map(item => ({ value: String(item.id), label: item.name }))
                        ]}
                  ...
                  {unmatched.map((file, index) => (
                      ...
                      <CustomDropdown
                        options={[
                          ...
                          ...menuItems.map(item => ({ value: String(item.id), label: item.name }))
                        ]}
