...
export default function BulkImageAssignment({
  ...
  const [generationSummary, setGenerationSummary] = useState({ success: 0, fail: 0, fallback: 0 });
  ...
  useEffect(() => {
    if (selectedItemIds.length === 1) {
      setSearchTargetId(selectedItemIds[0]);
      return;
    }
  ...
  const selectedItems = useMemo(() => allMenuItems.filter(item => selectedItemIds.includes(item.id)), [
    allMenuItems,
    selectedItemIds
  ]);
  ...
  async function fetchUniquePhoto(menuItem, usedPhotoIds) {
  ...
  function resolveScopeItems(scope, customIds = []) {
    ...
    switch (scope) {
      case 'selected':
        addItems(allMenuItems.filter(item => (customIds.length ? customIds : selectedItemIds).includes(item.id)));
        break;
      case 'filtered':
  ...
  async function handleAutoGenerateImages(options = {}) {
    ...
    try {
      ...
      const targetItems = overwriteExisting ? initialTargets : initialTargets.filter(item => !item.image_url);
      ...
      const summary = { success: 0, fail: 0, fallback: 0 };
      ...
      for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
        ...
        setBatchMessage(`Batch ${batchIndex + 1}/${totalBatches}: working on ${batch.length} item${batch.length === 1 ? '' : 's'}`);
        ...
        for (const menuItem of batch) {
          ...
          try {
            ...
            setGenerationStatus(prev => ({ ...prev, [menuItem.id]: 'fetching' }));
            ...
            if (!assignment) {
              ...
              summary.fail += 1;
              setGenerationStatus(prev => ({ ...prev, [menuItem.id]: 'skipped' }));
            } else {
              ...
              summary.success += 1;
              setGenerationStatus(prev => ({ ...prev, [menuItem.id]: assignment.duplicate ? 'duplicate' : 'success' }));
            }
            ...
            summary.fail += 1;
            setGenerationStatus(prev => ({ ...prev, [menuItem.id]: 'failed' }));
          }
        }
  ...
  async function handleBulkUpload() {
    ...
    try {
      ...
      for (const file of selectedFiles) {
        ...
        const result = await uploadMultipleImages([compressedFile]);
        if (result[0].success) {
      ...
      if (successful.length > 0) {
        toast.success(
          `${successful.length} image${successful.length === 1 ? '' : 's'} uploaded successfully${
            duplicates.length > 0 ? ` • ${duplicates.length} duplicate${duplicates.length === 1 ? '' : 's'} reused` : ''
          }`
        );
      ...
      const { matches: autoMatches, unmatched: unmatchedImages } = autoMatchImages(results, menuItems);
  ...
  function handleManualAssignment(fileIndex, menuItemId) {
  ...
  async function handleApplyAssignments() {
    ...
    Object.entries(manualAssignments).forEach(([fileIndex, menuItemId]) => {
      const file = unmatched[fileIndex];
      if (file && menuItemId) {
        assignments.push({
          menuItemId,
          imageUrl: file.url
  ...
  async function handleSearchPexels(page = 1) {
    if (!pexelsQuery.trim()) {
    ...
    try {
      ...
      const { photos, totalResults, nextPage } = await searchFoodPhotos(pexelsQuery, { perPage: 12, page });
  ...
  async function handleAssignFromSearch(photo) {
    const targetIds = selectedItemIds.length ? selectedItemIds : (searchTargetId ? [searchTargetId] : []);
    ...
    try {
      ...
      toast.success(`Assigned image to ${targetIds.length} item${targetIds.length === 1 ? '' : 's'}`);
      setGenerationStatus(prev => {
        const next = { ...prev };
  ...
  async function handleDeleteImages() {
    ...
    try {
      ...
      await onBulkDelete({
        scope: deleteScope,
        itemIds: deleteScope === 'selected' ? selectedItemIds : []
      });
    } finally {
  ...
  function renderStatusBadge(itemId) {
    const status = generationStatus[itemId];
  ...
  const missingCount = useMemo(() => allMenuItems.filter(item => !item.image_url).length, [allMenuItems]);
  ...
  const canStartGeneration = !generating && scopeCounts[generationScope] > 0;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6 overflow-hidden">
          ...
          <div>
              ...
              <p className="text-xs text-text-muted mb-3">
                {selectionCount} selected · {selectionPool.length} in view · {allMenuItems.length} total
              </p>
              ...
              <div data-overlay-scroll className="max-h-60 overflow-y-auto border border-theme-subtle rounded-lg">
                <ul className="divide-y divide-white/5 text-sm">
                  {selectionPool.map(item => (
                    <li key={item.id} className="flex items-center gap-2 px-3 py-2 hover:bg-white/5">
                      <input
                        type="checkbox"
                      ...
                      <div className="flex-1">
                        ...
                        {item.menu_categories?.name && (
                          <p className="text-[11px] text-text-muted">{item.menu_categories.name}</p>
                        )}
                      </div>
            ...
            <div className="border border-theme-subtle rounded-lg p-3 space-y-3 text-xs text-text-muted">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => handleAutoGenerateImages({ scope: 'selected', itemIds: selectedItemIds })}
              ...
              {TABS.map(tab => (
                <button
                  ...
                  className={`px-3 py-2 text-sm rounded-t-lg transition-colors ${
                    activeTab === tab.id ? 'bg-white/10 text-text-main font-semibold' : 'text-text-muted hover:text-text-main'
                  }`}
                >
                  {tab.label}
                </button>
              ...
              {activeTab === 'auto' && (
                <div className="space-y-6">
                  <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg space-y-2">
                    <h3 className="text-lg font-semibold text-text-main flex items-center gap-2">🪄 Smart Auto Generation</h3>
                    <p className="text-xs text-text-muted leading-relaxed">
                      Automatically pull unique, high-quality food photos from Pexels. Choose your scope, optionally overwrite existing
                      images, and monitor per-item progress with pause/resume controls.
                    </p>
                  </div>
                    ...
                    <div className="border border-theme-subtle rounded-lg p-4 space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {generating && !pauseGeneration && (
                          <button onClick={() => setPauseGeneration(true)} className="btn-outline flex items-center gap-2">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {generating && pauseGeneration && (
                          <button onClick={() => setPauseGeneration(false)} className="btn-outline flex items-center gap-2">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7-7 7M3 5l7 7-7 7" />
                            </svg>
                            Resume
                          </button>
                        )}
                  ...
                  >
                      ...
                      {(generating || batchMessage || isPaused) && (
                        <div className="bg-white/5 border border-theme rounded-lg p-3 text-xs text-text-muted space-y-2">
                          ...
                          {isPaused && <p className="text-yellow-400">Paused — press Resume to continue</p>}
                          <p>
                            Progress:{' '}
              ...
              {activeTab === 'search' && (
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-end">
                    <div className="flex-1">
                      <label className="text-xs uppercase text-text-muted block mb-1">Search Pexels</label>
                      <input
                        value={pexelsQuery}
                    ...
                    <button onClick={() => handleSearchPexels(1)} className="btn-primary px-4 py-2 text-sm" disabled={pexelsLoading}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {pexelsResults.map(photo => (
                      <div key={photo.id} className="group border border-theme rounded-lg overflow-hidden bg-white/5 flex flex-col">
                        ...
                        <div className="p-3 text-xs text-text-muted space-y-1 flex-1 flex flex-col">
                          <p className="font-semibold text-text-main text-sm line-clamp-1">Photo #{photo.id}</p>
                          {photo.photographer && <p className="line-clamp-1">by {photo.photographer}</p>}
                  ...
                  {pexelsResults.length === 0 && !pexelsLoading && (
                    <p className="text-sm text-text-muted">
                      Search results will appear here. Try cuisine or ingredient keywords (e.g. “thai curry dish”).
                    </p>
                  )}
                  ...
                  {uploadedImages.length === 0 && (
                    <div className="space-y-5">
                      <div className="flex items-center justify-between">
                        ...
                        <p className="text-xs text-text-muted">Upload and auto-match images by filename.</p>
                      </div>

                      <div
                        className={`border-2 border-dashed rounded-xl p-8 text-center transition-all ${
                          dragActive ? 'border-gold bg-gold bg-opacity-10' : 'border-gray-700 hover:border-gray-600'
              }`}
              ...
              {selectedFiles.length > 0 ? (
                <div>
                  <div className="text-5xl mb-4">📁</div>
                            <p className="text-text-main font-semibold mb-2">{selectedFiles.length} file(s) selected</p>
                ...
                <div>
                  <div className="text-6xl mb-4">📸</div>
                            <p className="text-text-main font-semibold mb-2">Drag & drop multiple images here</p>
                  <p className="text-sm text-text-muted mb-4">or</p>
                            <label className="btn-outline cursor-pointer inline-block text-sm px-4 py-2">
                    Browse Files
                              <input type="file" accept="image/*" multiple onChange={handleFileSelect} className="hidden" />
                  </label>
                            <p className="text-xs text-text-muted mt-4">Select multiple images at once (JPG, PNG, WEBP, GIF)</p>
                </div>
              )}
            </div>
        ...
        {uploadedImages.length > 0 && (
            ...
            {matches.length > 0 && (
                        <div className="space-y-3">
                          <h4 className="text-sm font-semibold text-green-400">✓ Auto-Matched ({matches.length})</h4>
                <div className="space-y-2">
                  {matches.map((match, index) => (
                              <div key={index} className="bg-green-900/20 border border-green-700 rounded-lg p-3 flex items-center gap-3">
                                <img src={match.file.url} alt={match.menuItem.name} className="w-12 h-12 object-cover rounded" />
                      <div className="flex-1">
                                  <p className="text-xs text-text-muted">{match.file.fileName}</p>
                                  <p className="text-sm text-text-main font-semibold">→ {match.menuItem.name}</p>
                      </div>
            ...
            {unmatched.length > 0 && (
                ...
                <div className="space-y-3">
                  {unmatched.map((file, index) => (
                              <div key={index} className="bg-yellow-900/20 border border-yellow-700 rounded-lg p-3 space-y-3">
            ...
            <div className="flex gap-3 border-t border-gray-800 pt-4">
                ...
                Apply {matches.length + Object.keys(manualAssignments).length} Assignment(s)
              </button>
            ...
            <li>• Name files similar to dish names (e.g., &quot;chicken-tikka.jpg&quot;)</li>
                ...
                <div className="space-y-6">
                  <div className="border border-theme-subtle rounded-lg p-4 space-y-4">
                    ...
                    <p className="text-xs text-text-muted">
                      Remove images in bulk based on your selection scope. This clears the stored URLs so you can regenerate fresh ones
                      immediately.
                    </p>
                    <div className="space-y-2 text-sm text-text-muted">
                      {DELETE_SCOPES.map(scope => (
                        <label key={scope.id} className="flex items-center gap-2 hover:text-text-main cursor-pointer">
                          <input
                            type="radio"
                            name="deleteScope"
                            value={scope.id}
                          ...
                          />
                          <span>
                            {scope.label} ({scopeCounts[scope.id]} items)
                          </span>
                        </label>
                      ))}
                    </div>
                    <button
                      ...
                      disabled={deleteInProgress || (deleteScope === 'selected' && selectedItemIds.length === 0)}
                    >
                      {deleteInProgress ? 'Deleting...' : 'Delete Images'}
                    </button>
                    <p className="text-[11px] text-text-muted/70">
                      Tip: Clear images first, then use Auto Generate to refill with brand-new photos.
                    </p>
                  </div>
                  ...
                  <div className="border border-theme-subtle rounded-lg p-4 space-y-3 text-xs text-text-muted">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        onClick={() => handleAutoGenerateImages({ scope: 'all', itemIds: allMenuItems.map(item => item.id) })}
                        className="btn-outline text-xs px-3 py-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    ...
                    <p>Remember to toggle &quot;Overwrite existing images&quot; if you want to replace current photos in your chosen scope.</p>
                  </div>
                </div>
              )}
            </div>
...
BulkImageAssignment.propTypes = {
  ...
  menuItems: PropTypes.arrayOf(
    PropTypes.shape({
      ...
      menu_categories: PropTypes.shape({
