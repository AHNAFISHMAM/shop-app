...
const CustomerProfileDrawer = ({ customer, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      ...
      try {
        ...
        if (customerId && customerEmail) {
          ordersQuery = ordersQuery.or(`user_id.eq.${customerId},customer_email.eq.${customerEmail}`)
        } else if (customerId) {
        ...
        const reservationsQuery = customerEmail
          ? supabase
              ...
              .select('id, reservation_date, reservation_time, status, party_size, table_number, occasion')
  ...
  const insightRows = useMemo(() => {
    ...
    return [
      ...
      {
        label: 'Last Order',
        value: customer.lastOrderAt ? dateTimeFormatter.format(customer.lastOrderAt) : '—'
      },
      {
        label: 'Last Visit',
        value: customer.lastVisitDate ? dateFormatter.format(customer.lastVisitDate) : '—'
      },
      {
        label: 'Status',
        value: customer.status.replace('-', ' ')
      }
    ]
  }, [customer])
  ...
  const handleSaveNotes = async () => {
    ...
    setUpdating(true)
    try {
      const { error: updateError } = await supabase.from('customers').update({ notes: noteDraft }).eq('id', customer.id)
    ...
    >
      ...
      >
        <header className="flex items-center justify-between border-b border-theme-subtle px-6 py-5">
          <div>
            <p className="text-xs uppercase tracking-[0.18em] text-[var(--text-main)]/40">Customer Profile</p>
            <h2 className="text-2xl font-semibold">{customer?.name || 'Unknown Guest'}</h2>
            {customer?.email && <p className="mt-1 text-sm text-[var(--text-main)]/50">{customer.email}</p>}
          </div>
        ...
        <div className="space-y-8 px-6 py-6">
          <section>
            <h3 className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--text-main)]/50">Insights</h3>
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              {insightRows.map(row => (
                <GlowPanel key={row.label} glow="soft" radius="rounded-lg" padding="p-4" background="bg-white/5" className="border-theme">
                  <p className="text-xs uppercase tracking-[0.12em] text-[var(--text-main)]/40">{row.label}</p>
          ...
          {hasPreferences && (
            <GlowPanel as="section" radius="rounded-lg" padding="p-5" background="bg-white/5" className="border-theme">
              <h3 className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--text-main)]/50">Preferences & Notes</h3>
              <div className="mt-3 space-y-3 text-sm text-[var(--text-main)]/70">
                {customer.location && (
                  <div>
                    <p className="text-xs uppercase tracking-[0.15em] text-[var(--text-main)]/40">Location</p>
                    <p className="mt-0.5">{customer.location}</p>
                  </div>
                )}
                {customer.dietaryRestrictions && customer.dietaryRestrictions.length > 0 && (
                  <div>
                    <p className="text-xs uppercase tracking-[0.15em] text-[var(--text-main)]/40">Dietary</p>
                    <div className="mt-1 flex flex-wrap gap-2">
                      {customer.dietaryRestrictions.map(tag => (
                        <span key={tag} className="rounded-full bg-amber-500/15 px-3 py-1 text-xs text-amber-200">
                          {tag}
                        </span>
                      ))}
                    </div>
                ...
                {customer.tags && customer.tags.length > 0 && (
                  <div>
                    <p className="text-xs uppercase tracking-[0.15em] text-[var(--text-main)]/40">Tags</p>
                    <div className="mt-1 flex flex-wrap gap-2">
                      {customer.tags.map(tag => (
                        <span key={tag} className="rounded-full bg-[var(--accent)]/15 px-3 py-1 text-xs text-[var(--accent)]">
                          {tag}
                        </span>
                      ))}
                    </div>
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.15em] text-[var(--text-main)]/40">Internal Notes</p>
                  <textarea
                    value={noteDraft}
          ...
          <section>
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--text-main)]/50">Recent Orders</h3>
              <span className="text-xs text-[var(--text-main)]/40">{orders.length} records</span>
            </div>
            <div className="mt-3 space-y-3">
              {loading ? (
                <div className="rounded-lg border border-theme bg-white/5 p-4 text-sm text-[var(--text-main)]/60">Loading…</div>
              ) : error ? (
                <div className="rounded-lg border border-rose-500/40 bg-rose-500/10 p-4 text-sm text-rose-200">{error}</div>
              ) : orders.length === 0 ? (
                <div className="rounded-lg border border-theme bg-white/5 p-4 text-sm text-[var(--text-main)]/60">No orders yet.</div>
              ) : (
                orders.map(order => (
                  <div
                    key={order.id}
                  ...
                  >
                    <div>
                      ...
                      <p className="mt-1 text-xs text-[var(--text-main)]/40">Order ID: {order.id}</p>
          ...
          <section>
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--text-main)]/50">Reservations</h3>
              <span className="text-xs text-[var(--text-main)]/40">{reservations.length} records</span>
            </div>
            <div className="mt-3 space-y-3">
              {loading ? (
                <div className="rounded-lg border border-theme bg-white/5 p-4 text-sm text-[var(--text-main)]/60">Loading…</div>
                ...
                reservations.map(reservation => (
                  ...
                  >
                    ...
                    {reservation.occasion && (
                      <p className="mt-1 text-xs text-[var(--text-main)]/50">Occasion: {reservation.occasion}</p>
                    )}
                  </div>
                ))
              )}
...
CustomerProfileDrawer.propTypes = {
  customer: PropTypes.shape({
    ...
    lastVisitDate: PropTypes.instanceOf(Date),
    ...
    lastOrderAt: PropTypes.instanceOf(Date),
