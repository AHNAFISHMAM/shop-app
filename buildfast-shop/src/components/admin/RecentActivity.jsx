  ...
  const fetchRecentActivity = async () => {
    try {
      ...
      const [{ data: orders }, { data: reservations }] = await Promise.all([
        ...
        supabase
          .from('reservations')
          .select('id, created_at, status, guest_name, party_size')
          .order('created_at', { ascending: false })
          .limit(2)
      ])
              ...
              <div className="flex-1">
                <p className="text-sm font-medium mb-1" style={{ color: 'var(--text-heading)' }}>
                  {activity.type === 'order' ? (
                    ...
                    <>
                      Reservation for{' '}
                      <span className="text-[var(--accent)]">{activity.guest_name || 'Guest'}</span>
                    </>
                  )}
                </p>
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <span style={{ color: 'var(--text-body-muted-light)' }}>
                    {activity.type === 'order' ? (
                      `৳${parseFloat(activity.order_total || 0).toFixed(2)}`
                    ) : (
