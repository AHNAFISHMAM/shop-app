...
export default function ImageUploadModal({ isOpen, onClose, onImageUploaded, dishName }) {
  ...
  useEffect(() => {
    ...
    async function loadLibrary() {
      try {
        ...
        setLibraryError(error.message || 'Unable to load image library');
        setLibraryImages([]);
      } finally {
        if (active) {
  ...
  function handleUseExisting(url) {
    if (!url) return;
    onImageUploaded(url);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-text-main">Upload Image</h2>
          <button
            onClick={handleClose}
            className="text-muted text-2xl leading-none transition-colors"
            style={{
              borderRadius: '0.5rem',
              padding: '0.25rem'
            }}
        ...
        <div className="mb-4">
          <p className="text-sm text-text-muted">
            Uploading image for: <span className="text-gold font-semibold">{dishName}</span>
          </p>
        </div>
        ...
        <div className="bg-dark-bg border border-gray-800 rounded-lg p-4 mb-4 space-y-4">
          ...
          <p className="text-[11px] text-text-muted">
            Keywords help when searching the media library later. Usage rights ensure future admins know where this asset may appear.
          </p>
        </div>
            ...
            {filteredLibrary.slice(0, 12).map(item => (
              ...
              >
                ...
                <div className="px-3 py-2">
                  <p className="text-xs font-semibold text-text-main line-clamp-2">
                    {item.name}
                  </p>
                  <p className="text-[11px] text-text-muted mt-1">
                    Tap to reuse this image
                  </p>
                </div>
              </button>
            ))}
