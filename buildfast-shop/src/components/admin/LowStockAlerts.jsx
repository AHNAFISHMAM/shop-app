 ...
 * - Real-time dish and variant availability monitoring
...
function LowStockAlerts() {
  ...
  const [dishCombinations, setDishCombinations] = useState({})
  ...
  const fetchCombinationsForDishes = useCallback(async (dishes) => {
    const combosMap = {}
    ...
    for (const dish of dishes) {
      try {
        ...
        if (!error && data) {
          ...
          let filteredCombos = data
          if (!showAllDishes) {
            filteredCombos = data.filter(combo => combo.stock_quantity <= threshold)
          }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('all-products-availability')
  ...
  const handleReplenish = async (dishId) => {
    ...
    try {
      ...
      const newQuantity = (dish.stock_quantity || 0) + addQuantity
      ...
      if (combosError) {
        ...
        const updatePromises = combinations.map(combo => {
          const newComboQuantity = (combo.stock_quantity || 0) + addQuantity
          return supabase
            .from('variant_combinations')
            .update({ stock_quantity: newComboQuantity })
            .eq('id', combo.id)
        })
      ...
      setMessage({
        type: 'success',
        text: `${dish.name} replenished successfully. Availability updated: ${dish.stock_quantity} → ${newQuantity} portions`
      })
  ...
  const handleReplenishCombination = async (comboId, dishName, variantLabel) => {
    ...
    try {
      ...
      const newQuantity = (combo.stock_quantity || 0) + addQuantity
      ...
      setMessage({
        type: 'success',
        text: `${dishName} (${variantLabel}) replenished successfully. Availability: ${combo.stock_quantity} → ${newQuantity} portions`
      })
  ...
  const formatVariantLabel = (variantValues) => {
  ...
  const getDishStatus = (dish) => {
    const threshold = dish.low_stock_threshold || 10
    if (dish.stock_quantity === 0) return 'unavailable'
    if (dish.stock_quantity <= threshold) return 'limited'
    return 'available'
  }

  const statusCounts = allDishes.reduce((acc, dish) => {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        {allDishes.map((dish) => {
          const threshold = dish.low_stock_threshold || 10
          const availabilityPercentage = Math.min((dish.stock_quantity / threshold) * 100, 100)
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mb-2">
                      <div className="text-sm">
                        ...
                        <span style={{ color: 'var(--text-body-muted)' }}> / {threshold} portions</span>
                      </div>
                    </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      value={inputValue}
                      onChange={(e) => setReplenishInput(prev => ({ ...prev, [dish.id]: e.target.value }))}
                  ...
                  <p className="text-xs mt-2" style={{ color: 'var(--text-body-muted)' }}>
                    New total: <span className="font-semibold" style={{ color: 'var(--text-heading)' }}>
                      {dish.stock_quantity + parseInt(inputValue || 0)} portions
                    </span>
                  </p>
                </div>
              </div>
              ...
              {hasVariants && isExpanded && (
                <div className="ml-4 space-y-2">
                  {combinations.map((combo) => {
                    ...
                    const comboUnavailable = combo.stock_quantity === 0
                    const comboLimited = combo.stock_quantity > 0 && combo.stock_quantity <= threshold
                      ...
                      >
                        <div className="flex items-center justify-between gap-2 mb-2">
                          ...
                          <span className="text-sm font-bold" style={{ color: comboStatusColor }}>
                            {combo.stock_quantity} / {threshold}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onChange={(e) => setReplenishInput(prev => ({ ...prev, [comboId]: e.target.value }))}
