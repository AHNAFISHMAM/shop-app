 ...
 */
const ProductCard = memo(({ product, onAddToCart, getImageUrl, enableCustomization = false, compact = false }: ProductCardProps) => {
  ...
  const isFeatured = useMemo(() => product.is_featured || product.chef_special, [product.is_featured, product.chef_special]);
  ...
  const reviewSnippet = useMemo(() => {
    return product.review_snippet || product.reviewSnippet || product.review_highlight || product.reviewHighlight || null;
  }, [product.review_snippet, product.reviewSnippet, product.review_highlight, product.reviewHighlight]);
  ...
  const isLowStock = useMemo(() => {
    return !isOutOfStock && product.stock_quantity !== undefined && product.stock_quantity <= (product.low_stock_threshold || 10);
  ...
  return (
    ...
    >
            ...
            <m.div
              ...
              className={`absolute bg-[var(--accent)] text-black rounded-md text-sm font-bold flex items-center gap-1 z-10 min-h-[44px] min-w-[44px] ${
                compact
                  ...
                  : 'top-2 left-2 px-2 py-1 sm:text-base'
              }`}
            ...
            <m.div
              ...
              className={`absolute bg-[var(--status-error-bg)] text-[var(--color-red)] rounded-md text-sm font-bold z-10 min-h-[44px] min-w-[44px] ${
                compact
                  ...
                  : 'top-2 right-2 px-2 py-1 sm:text-base'
              }`}
              initial={prefersReducedMotion ? undefined : { opacity: 0, scale: 0.8, x: 10 }}
              animate={prefersReducedMotion ? undefined : { opacity: 1, scale: 1, x: 0 }}
              exit={prefersReducedMotion ? undefined : { opacity: 0, scale: 0.8 }}
      ...
      <div className={`flex flex-col flex-1 ${compact ? 'px-2 sm:px-3 py-1.5 sm:py-2' : 'px-4 sm:px-6 py-3 sm:py-4'}`}>
        <h3 id={`product-${product.id}-name`} className={`font-bold text-[var(--text-main)] line-clamp-2 ${
          compact
            ...
            <>
              {product.dietary_tags.slice(0, 3).map((tag, index) => (
                <m.span
                  ...
                  transition={prefersReducedMotion ? { duration: 0 } : { duration: 0.3, delay: 0.15 + index * 0.05 }}
          ...
          >
            {allergenBadges.slice(0, 4).map((tag, index) => (
              <m.span
                ...
                initial={prefersReducedMotion ? undefined : { opacity: 0, scale: 0.8, y: 5 }}
                animate={prefersReducedMotion ? undefined : { opacity: 1, scale: 1, y: 0 }}
                transition={prefersReducedMotion ? { duration: 0 } : { duration: 0.3, delay: 0.3 + index * 0.05 }}
                whileHover={prefersReducedMotion ? undefined : { scale: 1.05, y: -2 }}
        ...
        {reviewSnippet && (
          <p className={`text-sm italic text-[var(--text-main)]/70 line-clamp-2 ${
        ...
        {enableCustomization && (
          ...
          >
            <m.button
              ...
              aria-label={customizerOpen ? 'Close customization options' : 'Open customization options'}
            >
              <span>Customize</span>
              <m.svg
                className="h-4 w-4"
            ...
            <AnimatePresence>
              {customizerOpen && (
                ...
                >
                  ...
                  >
                    <p className="text-sm sm:text-base font-semibold uppercase tracking-[0.2em] text-[var(--text-muted)]">Add-ons</p>
                    <div className={`flex flex-wrap ${
                      compact ? 'gap-1.5 sm:gap-2' : 'gap-3 sm:gap-4'
                    }`} role="group" aria-label="Add-on options">
                        ...
                        return (
                          <m.button
                            ...
                            className={`rounded-full border text-sm font-medium transition-all min-h-[44px] min-w-[44px] flex items-center justify-center ${
                              compact
                                ? 'px-2 sm:px-3 py-1.5'
                                : 'px-4 sm:px-6 py-3 sm:text-base'
                            } ${
                              isActive
                  ...
                  <m.div
                    ...
                    transition={prefersReducedMotion ? { duration: 0 } : { duration: 0.3, delay: 0.2 }}
                  >
                    <p className="text-sm sm:text-base font-semibold uppercase tracking-[0.2em] text-[var(--text-muted)]">Spice Level</p>
        ...
        <div className={`flex items-center justify-between mt-auto ${
          ...
          <div className={`font-bold text-[var(--accent)] ${
            ...
            {getCurrencySymbol(product.currency || 'BDT')}{formatPrice(Number(product.price), 0)}
          </div>
          ...
          <m.button
            ...
            whileHover={!isOutOfStock && !prefersReducedMotion ? { scale: 1.06, y: -2 } : undefined}
            ...
            aria-label={isOutOfStock ? `${product.name} is unavailable` : `Add ${product.name} to cart`}
