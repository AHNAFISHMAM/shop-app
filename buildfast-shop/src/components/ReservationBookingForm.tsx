...
const roomOptions: RoomOption[] = [
  { value: 'standard', label: 'Standard Room', description: 'Queen bed - Cozy interior - Perfect for quick stays' },
  { value: 'deluxe', label: 'Deluxe Room', description: 'King bed - City view - Lounge access included' },
  { value: 'suite', label: 'Executive Suite', description: 'Separate living area - Premium minibar - Butler service' }
];
...
const ReservationBookingForm = ({
  ...
  const validate = (): boolean => {
    ...
    if (!formData.guests || Number(formData.guests) < minGuests || Number(formData.guests) > maxGuests) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid gap-3 sm:gap-4 md:gap-6 md:grid-cols-2">
        <div className="space-y-2">
          <label className="text-sm sm:text-base font-medium text-[var(--text-main)]">Full Name</label>
          <input
            type="text"
            ...
            required
          />
          {errors.fullName && <p className="text-[10px] sm:text-xs text-rose-400">{errors.fullName}</p>}
        </div>

        <div className="space-y-2">
          <label className="text-sm sm:text-base font-medium text-[var(--text-main)]">Email Address</label>
          <input
            type="email"
        ...
        <div className="space-y-2">
          <label className="text-sm sm:text-base font-medium text-[var(--text-main)]">Phone Number</label>
          <input
            type="tel"
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            onChange={(event: ChangeEvent<HTMLInputElement>) => updateField('guests', Number(event.target.value))}
            className="w-full min-h-[44px] rounded-xl sm:rounded-2xl border border-theme bg-theme-elevated px-4 sm:px-6 py-3 text-sm sm:text-base text-[var(--text-main)] focus:border-[var(--accent)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/40"
            disabled={disabled || submitting}
            required
          />
      ...
      <div className="grid gap-3 sm:gap-4 md:gap-6 md:grid-cols-3">
        <div className="space-y-2">
          <label className="text-sm sm:text-base font-medium text-[var(--text-main)]">Check-in</label>
          <input
            type="date"
            min={today}
            ...
            required
          />
          {errors.checkInDate && <p className="text-[10px] sm:text-xs text-rose-400">{errors.checkInDate}</p>}
        </div>

        <div className="space-y-2">
          <label className="text-sm sm:text-base font-medium text-[var(--text-main)]">Check-out</label>
          <input
            type="date"
            min={formData.checkInDate || today}
            value={formData.checkOutDate}
            onChange={(event: ChangeEvent<HTMLInputElement>) => updateField('checkOutDate', event.target.value)}
            className="w-full min-h-[44px] rounded-xl sm:rounded-2xl border border-theme bg-theme-elevated px-4 sm:px-6 py-3 text-sm sm:text-base text-[var(--text-main)] focus:border-[var(--accent)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/40"
            disabled={disabled || submitting}
            required
          />
          {errors.checkOutDate && <p className="text-[10px] sm:text-xs text-rose-400">{errors.checkOutDate}</p>}
        </div>

        <div className="space-y-2">
          <label className="text-sm sm:text-base font-medium text-[var(--text-main)]">Arrival Time</label>
          <input
            type="time"
            value={formData.checkInTime}
            onChange={(event: ChangeEvent<HTMLInputElement>) => updateField('checkInTime', event.target.value)}
            className="w-full min-h-[44px] rounded-xl sm:rounded-2xl border border-theme bg-theme-elevated px-4 sm:px-6 py-3 text-sm sm:text-base text-[var(--text-main)] focus:border-[var(--accent)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/40"
            disabled={disabled || submitting}
            required
          />
          {errors.checkInTime && <p className="text-[10px] sm:text-xs text-rose-400">{errors.checkInTime}</p>}
        </div>
      </div>
        ...
        <div className="space-y-3 sm:space-y-4">
          {roomOptions.map((option) => (
            <label
              ...
              style={formData.roomType !== option.value ? {
            ...
            >
              <input
                ...
                onChange={(event: ChangeEvent<HTMLInputElement>) => updateField('roomType', event.target.value)}
                disabled={disabled || submitting}
                required
              />
              <div>
                <p className="text-sm sm:text-base font-semibold text-[var(--text-main)]">{option.label}</p>
                <p className="text-[10px] sm:text-xs text-[var(--text-main)]/60">{option.description}</p>
              </div>
            </label>
          ))}
        </div>
        {errors.roomType && <p className="text-[10px] sm:text-xs text-rose-400">{errors.roomType}</p>}
      </div>

      <div className="space-y-2">
        <label className="text-sm sm:text-base font-medium text-[var(--text-main)]">Special Requests</label>
        <textarea
          value={formData.requests}
          onChange={(event: ChangeEvent<HTMLTextAreaElement>) => updateField('requests', event.target.value)}
      ...
      <div className="rounded-xl sm:rounded-2xl border border-theme bg-theme-elevated px-4 sm:px-6 md:px-10 py-3 sm:py-4 md:py-5 text-[10px] sm:text-xs text-[var(--text-main)]/70">
        By submitting this reservation you agree to the {propertyName} stay policies. A confirmation email will be sent
        instantly and our concierge may reach out by phone for any clarifications.
      </div>
