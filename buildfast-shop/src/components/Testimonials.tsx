...
const Testimonials = () => {
  ...
  const reviewsEnabled = useMemo(() => settings?.show_public_reviews ?? false, [settings?.show_public_reviews]);
  const testimonialsEnabled = useMemo(() => settings?.show_home_testimonials ?? true, [settings?.show_home_testimonials]);
  ...
  if (loading) {
    return (
      <div className="grid md:grid-cols-3 gap-6" role="status" aria-live="polite" aria-label="Loading testimonials">
        {[0, 1, 2].map((i) => (
          <div key={`testimonial-skeleton-${i}`} className="card-soft space-y-4 animate-pulse">
            ...
            <div className="flex items-center gap-3 pt-2">
              <div className="h-12 w-12 rounded-full min-h-[44px] min-w-[44px]" style={{ backgroundColor: skeletonBg }} />
  ...
  return (
    <>
          ...
          return (
            ...
            >
              <div className="flex gap-1" role="img" aria-label={`${stars} out of 5 stars`}>
                {[...Array(stars)].map((_, i) => (
                  <svg key={`${testimonial.id}-star-${i}`} className="w-5 h-5 text-[var(--accent)] min-h-[44px] min-w-[44px]" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
                {[...Array(5 - stars)].map((_, i) => (
                  <svg key={`${testimonial.id}-star-empty-${i}`} className="w-5 h-5 text-[var(--text-muted)] min-h-[44px] min-w-[44px]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <div className="flex items-center gap-3 pt-2">
                <div className="w-12 h-12 rounded-full bg-[var(--accent)] flex items-center justify-center text-black font-bold min-h-[44px] min-w-[44px]" aria-label={`Avatar for ${testimonial.name}`}>
                  {testimonial.avatar}
                </div>
                <div>
      ...
      {usingFallback && isAdmin && (
        <p className="mt-4 text-sm text-[var(--text-muted)] text-center" role="status" aria-live="polite">
