...
async function fetchUserCartItemsWithoutJoins(userId) {
  try {
    ...
    // Extract product IDs
    const menuItemIds = [
      ...new Set(cartItems.filter(item => item.menu_item_id).map(item => item.menu_item_id)),
    ]
    const dishIds = [
      ...new Set(cartItems.filter(item => item.product_id).map(item => item.product_id)),
    ]
...
async function getGuestCartItems() {
  ...
  try {
    ...
    // Extract product IDs
    const menuItemIds = [
      ...new Set(guestCart.filter(item => item.menu_item_id).map(item => item.menu_item_id)),
    ]
    const dishIds = [
      ...new Set(guestCart.filter(item => item.product_id).map(item => item.product_id)),
    ]
    ...
    if (menuItemIds.length > 0) {
      try {
        ...
        if (menuError) {
          ...
          logger.warn(
            'Error fetching menu items for guest cart (table might not exist):',
            menuError
          )
    ...
    const productIds = [
      ...new Set(
        guestCart.filter(item => item.product_id && !item.menu_item_id).map(item => item.product_id)
      ),
    ]
    let productsMap = {}
    ...
    return guestCart.map(item => {
      ...
      const resolvedProductType = menuItem
        ? 'menu_item'
        : dish
          ? 'dish'
