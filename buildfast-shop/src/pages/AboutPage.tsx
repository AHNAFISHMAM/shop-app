    ...
    >
      ...
      >
        ...
        <div className="hero-container relative text-center space-y-4 sm:space-y-6 px-4">
          <span className="inline-flex items-center gap-2 rounded-full border border-[var(--accent)]/30 bg-[var(--accent)]/10 px-3 py-1 sm:px-4 sm:py-2 text-sm sm:text-xs tracking-[0.2em] uppercase text-[var(--accent)]" role="status" aria-label="About Star Café">
            About Star Café
          </span>
          <h1 id="about-hero-heading" className="text-3xl sm:text-4xl md:text-5xl font-bold leading-tight" style={{ color: 'var(--text-main)' }}>
          ...
          <div className="grid gap-4 sm:gap-6 md:grid-cols-2 max-w-3xl mx-auto" role="list" aria-label="Purpose and promise">
            {purposeHighlights.map((item, index) => (
              <article
                ...
                role="listitem"
              >
                <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.12),transparent_60%)]" aria-hidden="true" />
                <div className="relative space-y-2">
                  <h3 className="text-sm font-semibold uppercase tracking-wide text-[var(--accent)]">{item.title}</h3>
      ...
      <section aria-labelledby="values-heading">
        ...
        >
          <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.12),transparent_55%)]" aria-hidden="true" />
          <div className="relative space-y-8">
            <div className="text-center space-y-2 sm:space-y-3">
              <p className="text-sm sm:text-xs uppercase tracking-[0.3em] sm:tracking-[0.35em] text-[var(--accent)]/80">Guiding Principles</p>
              <h2 id="values-heading" className="text-2xl sm:text-3xl md:text-4xl font-bold leading-tight" style={{ color: 'var(--text-main)' }}>
                Our Values
              </h2>
            ...
            <div className="grid gap-4 sm:gap-6 md:gap-8 sm:grid-cols-2 lg:grid-cols-3" role="list" aria-label="Our values">
              {ourValues.map((value, index) => (
                <article
                  ...
                  role="listitem"
                >
                  <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.1),transparent_65%)] opacity-90 transition-opacity duration-300 group-hover:opacity-100" aria-hidden="true" />
                  <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(120%_100%_at_50%_0%,rgba(255,255,255,0.05),transparent)] opacity-0 transition-opacity duration-300 group-hover:opacity-100" aria-hidden="true" />
                  <div className="relative space-y-3">
                    <span className="inline-flex items-center rounded-full bg-[var(--accent)]/15 px-4 py-1 text-xs font-medium uppercase tracking-[0.2em] text-[var(--accent)]/80" role="status">
                      {value.title}
                    </span>
      ...
      >
        ...
        <SectionTitle
          ...
          align="center"
        />
        <h2 id="gallery-heading" className="sr-only">Gallery of Star Café</h2>

        {loading ? (
          <div className="flex justify-center items-center py-12" role="status" aria-live="polite" aria-label="Loading gallery">
            <div className="animate-spin rounded-full h-12 w-12 border-4 border-[var(--accent)] border-t-transparent" aria-hidden="true"></div>
            <span className="sr-only">Loading gallery images...</span>
          </div>
        ) : error ? (
          <div className="text-center py-12" role="alert" aria-live="assertive">
            <div
              className="max-w-md mx-auto p-6 rounded-xl border border-[var(--status-error-border)] bg-[var(--status-error-bg)]"
            >
              <svg className="w-12 h-12 mx-auto mb-4 text-[var(--color-red)]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
              <p className="text-sm mb-2 text-[var(--color-red)] font-semibold">Gallery Loading Error</p>
              <p className="text-xs text-muted">{error}</p>
            </div>
          </div>
        ) : galleryCards.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mt-6 sm:mt-8" role="list" aria-label="Gallery images">
