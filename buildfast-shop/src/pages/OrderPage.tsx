...
const OrderPage = memo((): JSX.Element => {
  ...
  // State management
  const [addingToCart, setAddingToCart] = useState<Record<string | number, boolean>>({})
  const [successMessage, setSuccessMessage] = useState<Record<string | number, string>>({})
  ...
  const totalCartQuantity = useMemo(() => {
    return cartItems.reduce((sum: number, item: { quantity?: number }) => sum + (item.quantity || 0), 0)
  }, [cartItems])

  // Core add to cart logic - no event required
  const handleAddToCart = useCallback(async (meal: Meal, event: React.MouseEvent | null = null): Promise<void> => {
  ...
  const handleAddToCartForViews = useCallback(async (meal: Meal): Promise<void> => {
                    ...
                    >
                      {[1, 2, 3].map((section) => (
                        <div key={section} className="space-y-4 animate-pulse">
                          <div className="h-8 bg-[var(--bg-elevated)] rounded w-48" aria-hidden="true"></div>
                      ...
                      >
                        <m.div
                          ...
                          style={{
                            ...
                            borderColor: isLightTheme ? 'rgba(var(--text-main-rgb), 0.15)' : undefined
                          }}
                          aria-hidden="true"
                        >
                          <m.svg
                        ...
                        {mealGridBatches.map((batch, batchIndex) => {
                          const batchMotionProps = batchIndex === 0
                            ...
                            : {
                                ...
                                viewport: prefersReducedMotion ? undefined : { once: true, amount: 0.25, margin: '40px 0px 0px 0px' },
                                exit: prefersReducedMotion ? undefined : 'exit',
                              }
                              ...
                              {batch.map((meal) => {
                                return (
                                  <m.div
                                    ...
                                    initial={prefersReducedMotion ? undefined : "hidden"}
                                    animate={batchIndex === 0 && !prefersReducedMotion ? 'visible' : undefined}
                                    whileInView={batchIndex === 0 || prefersReducedMotion ? undefined : 'visible'}
                                    viewport={
