...
const OrderHistory = memo((): JSX.Element | null => {
  ...
  const [orderItems, setOrderItems] = useState<Record<string, unknown[]>>({}) // Map of orderId -> items
  ...
  const [returnRequests, setReturnRequests] = useState<Record<string, unknown>>({}) // Map of orderId -> returnRequest
  ...
  const [selectedOrderForReturn, setSelectedOrderForReturn] = useState<unknown | null>(null)
  ...
  const [feedbackSubmitted, setFeedbackSubmitted] = useState<Record<string, { rating: number; note: string }>>({})
  ...
  const handleFeedbackSubmit = useCallback(async (orderId: string): Promise<void> => {
  ...
  const formatCurrency = (value: number | string | null | undefined): string => `${CURRENCY_SYMBOL}${Number(value || 0).toFixed(2)}`
  ...
  const fetchOrders = useCallback(async (page: number = 1, reset: boolean = false) => {
    ...
    try {
      ...
      let query = supabase
        ...
        .select('id, status, order_total, created_at, customer_email, customer_name', { count: 'exact' })
        .eq('user_id', user.id)
  ...
  const fetchOrderItems = async (orderId: string): Promise<unknown[]> => {
  ...
  const isReturnEligible = useCallback((order: { id: string; status: string; created_at: string }): boolean => {
    // Must be delivered
  ...
  const getReturnStatusColor = useCallback((status: string): string => {
     const colors: Record<string, string> = {
      requested: 'bg-[var(--status-warning-bg)] text-[var(--color-amber)] border-[var(--status-warning-border)]',
      approved: 'bg-[rgba(var(--color-blue-rgb),0.1)] text-[var(--color-blue)] border-[rgba(var(--color-blue-rgb),0.3)]',
      denied: 'bg-[var(--status-error-bg)] text-[var(--color-red)] border-[var(--status-error-border)]',
      received: 'bg-[rgba(var(--color-purple-rgb),0.1)] text-[var(--color-purple)] border-[rgba(var(--color-purple-rgb),0.3)]',
      refunded: 'bg-[var(--status-success-bg)] text-[var(--color-emerald)] border-[var(--status-success-border)]'
    }
  ...
  const handleSubmitReturnRequest = async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
  ...
  const getStatusColor = useCallback((status: string): string => {
     const colors: Record<string, string> = {
      pending: 'bg-[var(--status-warning-bg)] text-[var(--color-amber)] border-[var(--status-warning-border)]',
      paid: 'bg-[var(--status-success-bg)] text-[var(--color-emerald)] border-[var(--status-success-border)]',
      processing: 'bg-[rgba(var(--color-blue-rgb),0.1)] text-[var(--color-blue)] border-[rgba(var(--color-blue-rgb),0.3)]',
      shipped: 'bg-[rgba(var(--color-purple-rgb),0.1)] text-[var(--color-purple)] border-[rgba(var(--color-purple-rgb),0.3)]',
      delivered: 'bg-[var(--status-success-bg)] text-[var(--color-emerald)] border-[var(--status-success-border)]',
      failed: 'bg-[var(--status-error-bg)] text-[var(--color-red)] border-[var(--status-error-border)]',
  ...
  const getProductImage = useCallback((product: { image_url?: string; images?: string[] } | null | undefined): string => {
    if (!product) {
  ...
  const getVariantDisplay = useCallback((orderItem: { variant_details?: unknown }): string | null => {
  ...
  const handleReorder = async (order: { id: string }): Promise<void> => {
  ...
  const handleDownloadInvoice = (order: { id: string; created_at: string; customer_name?: string; customer_email: string; order_total?: number; status: string }): void => {
...
Thank you for your order!
  ...
  const activeOrders = useMemo(() => orders.filter(order => activeStatuses.has(order.status)).length, [orders, activeStatuses])
  const deliveredOrders = useMemo(() => orders.filter(order => order.status === 'delivered').length, [orders])
  const totalSpent = useMemo(() => orders.reduce((sum, order) => sum + (order.order_total || 0), 0), [orders])
          ...
          <>
            {enableLoyalty && loyalty && (
              <div className="glow-surface glow-soft relative overflow-hidden rounded-xl sm:rounded-2xl border border-theme bg-[var(--bg-main)] px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 mb-6 sm:mb-8">
                <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(var(--accent-rgb),0.25),transparent_60%)]" aria-hidden="true" />
                <div className="relative z-10 flex flex-col gap-4 sm:gap-6 lg:gap-8 lg:flex-row lg:items-center lg:justify-between">
                  <div className="space-y-3 sm:space-y-4 max-w-xl">
                    <p className="text-sm sm:text-xs uppercase tracking-[0.3em] text-[var(--color-amber)]/80">Loyalty Snapshot</p>
                  ...
                  {referral && (
                    <div className="flex w-full max-w-sm flex-col gap-4 sm:gap-6 rounded-xl sm:rounded-2xl border border-theme bg-theme-elevated px-4 sm:px-6 lg:px-8 py-4 sm:py-6 backdrop-blur-sm">
                      <div className="space-y-2">
                        ...
                        <p className="text-sm sm:text-xs text-[var(--text-main)]/70">{referral.subcopy}</p>
                      </div>
                      <div className="flex items-center justify-between rounded-xl sm:rounded-2xl border border-theme bg-[var(--bg-main)]/30 px-4 sm:px-6 py-3">
                        <span className="text-sm sm:text-xs uppercase tracking-[0.2em] text-[var(--text-main)]/60">Code</span>
                        <span className="font-semibold text-[var(--color-amber)] text-sm sm:text-base">{referral.code}</span>
                      </div>
                      <button
                        type="button"
          ...
          {quickActions.map((action, index) => (
            ...
            >
              ...
              <span className="space-y-1">
                ...
                <span className="block text-sm sm:text-xs text-muted">{action.description}</span>
              </span>
            </button>
          ))}
        </div>
        ...
        {returnSuccess && (
          <div className="mb-6 sm:mb-8 rounded-xl sm:rounded-2xl border-[var(--status-success-border)] bg-[var(--status-success-bg)] px-4 sm:px-6 lg:px-8 py-4 sm:py-6 animate-fade-in">
            <div className="flex items-center gap-3 sm:gap-4">
              <svg className="h-5 w-5 sm:h-6 sm:w-6 text-[var(--color-emerald)] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              </svg>
              <p className="text-sm sm:text-base font-medium text-[var(--color-emerald)]" role="status" aria-live="polite">
          ...
          >
            <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
              ...
              >
                ...
                <p className="text-sm sm:text-xs uppercase tracking-[0.2em] text-muted">Based on your recent orders</p>
              </div>
              <button
         ...
         {orders.length > 0 && (
          ...
          >
            ...
            <div className="relative">
              ...
              />
              <span id="search-helper" className="sr-only">Search by order number or product name</span>
              {searchQuery && (
                <button
                ...
                >
                  <div className="px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
                    ...
                    <div className="flex flex-col gap-4 sm:gap-6 lg:flex-row lg:items-center lg:justify-between">
                      <div className="flex-1 space-y-2 sm:space-y-3" id={`order-header-${order.id}`}>
                        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                          <span className="text-sm sm:text-base font-mono font-semibold uppercase tracking-wider text-[var(--text-main)]" aria-label={`Order number ${order.id.substring(0, 8)}`}>
                            #{order.id.substring(0, 8)}
                          </span>
                          <span className={`inline-flex rounded-full border px-3 py-1 text-sm sm:text-xs font-semibold uppercase tracking-wider ${getStatusColor(order.status)}`} role="status" aria-label={`Order status: ${order.status}`}>
                            {order.status}
                          </span>
                          {returnRequest && (
                            <span className={`inline-flex rounded-full border px-3 py-1 text-sm sm:text-xs font-semibold uppercase tracking-wider ${getReturnStatusColor(returnRequest.status)}`} role="status" aria-label={`Return status: ${returnRequest.status}`}>
                    ...
                    {/* Expanded Details */}
                    {isExpanded && (
                      <div id={`order-details-${order.id}`} className="mt-6 sm:mt-8 space-y-6 sm:space-y-8 border-t border-theme pt-6 sm:pt-8" role="region" aria-labelledby={`order-header-${order.id}`}>
                        {/* Order Items */}
                        <div className="space-y-4 sm:space-y-6">
                          <h3 className="text-base sm:text-lg font-semibold text-[var(--text-main)]" id={`order-items-${order.id}`}>Order Items</h3>

                          {isLoadingItems ? (
                            <div className="flex items-center justify-center py-8 sm:py-12" role="status" aria-live="polite" aria-label="Loading order items">
                              <div className="inline-flex h-8 w-8 animate-spin rounded-full border-4 border-[var(--accent)] border-t-transparent" aria-hidden="true"></div>
                              <span className="sr-only">Loading order items...</span>
                            </div>
                          ) : itemsLoadError ? (
                            <div className="rounded-lg border-[var(--status-error-border)] bg-[var(--status-error-bg)] px-4 sm:px-6 lg:px-8 py-4 sm:py-6 text-sm sm:text-base text-[var(--color-red)]" role="alert" aria-live="assertive">
                              {itemsLoadError}
                            </div>
                          ) : items.length === 0 ? (
                                  ...
                                  >
                                    ...
                                    <div className="flex flex-1 flex-col justify-between gap-2 sm:gap-3">
                                      <div className="space-y-1 sm:space-y-2">
                                        ...
                                        {variantText && (
                                          <p className="text-sm sm:text-xs text-muted">{variantText}</p>
                                        )}
                                        {item.customizations && (
                        ...
                        {enableOrderFeedback && order.status === 'delivered' && (
                          <div className="rounded-xl sm:rounded-2xl border border-theme bg-gradient-to-br from-[rgba(var(--accent-rgb),0.08)] to-[rgba(255,255,255,0.02)] px-4 sm:px-6 py-3 sm:py-4">
                            <button
                              ...
                              aria-label={isFeedbackOpen ? 'Close feedback form' : 'Open feedback form'}
                            >
                              <div>
                                <h3 id={`feedback-heading-${order.id}`} className="text-base sm:text-lg font-semibold text-[var(--text-main)]">
                            ...
                            {isFeedbackOpen && !feedbackData && (
                              <div id={`feedback-form-${order.id}`} className="mt-4 space-y-4" role="region" aria-labelledby={`feedback-heading-${order.id}`}>
                                <div>
                                  <label htmlFor={`feedback-rating-${order.id}`} className="mb-2 block text-sm sm:text-base font-semibold text-[var(--text-main)]">
                                    Rating
                                  </label>
                                  <div className="flex gap-2" role="radiogroup" aria-labelledby={`feedback-rating-${order.id}`}>
                                    {[1, 2, 3, 4, 5].map((rating) => (
                                      <button
                                        ...
                                        role="radio"
                                        aria-checked={(feedbackRatings[order.id] || 0) >= rating}
                                      >
                                        ★
                                      </button>
                                    ))}
      ...
      {returnModalOpen && selectedOrderForReturn && (
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmitReturnRequest} className="space-y-4 sm:space-y-6 px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 text-[var(--text-main)] relative z-10 pointer-events-auto">
              {returnError && (
                <div className="rounded-lg border-[var(--status-error-border)] bg-[var(--status-error-bg)] px-4 sm:px-6 lg:px-8 py-3 sm:py-4 text-sm sm:text-base text-[var(--color-red)]" role="alert" aria-live="assertive">
                  {returnError}
                </div>
              )}
