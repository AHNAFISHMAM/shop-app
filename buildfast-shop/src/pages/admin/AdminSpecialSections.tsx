...
const AdminSpecialSections = () => {
  ...
  const toggleSection = async (section: SpecialSection) => {
    ...
    setSections(prev =>
      prev.map(item => (item.id === section.id ? { ...item, is_available: optimisticValue } : item))
    )
  ...
  const generateMessage = async (section: SpecialSection) => {
    ...
    setSections(prev =>
      prev.map(item => (item.id === section.id ? { ...item, custom_message: randomMsg } : item))
    )
      ...
      <div className="space-y-8 pb-20">
        ...
        <div className="rounded-3xl border border-theme bg-[rgba(6,8,12,0.95)] p-6 shadow-[0_18px_48px_rgba(0,0,0,0.4)]">
          <h1 className="text-3xl font-bold text-[var(--text-main)]">
            Special Sections - Super Simple!
          </h1>
          <p className="mt-2 text-[var(--text-main)]/60">
            ✅ Turn sections ON/OFF &nbsp;|&nbsp; ✨ Generate messages &nbsp;|&nbsp; ☑️ Check boxes
            to add dishes
          </p>
        </div>
        ...
        >
          {sections.map((section, index) => (
            ...
            >
              ...
              <p className="text-sm text-[var(--text-main)]/60 italic mb-3">
                &ldquo;{section.custom_message}&rdquo;
              </p>
        ...
        <div className="rounded-3xl border border-theme bg-[rgba(6,8,12,0.95)] shadow-[0_20px_60px_rgba(0,0,0,0.4)] overflow-hidden overflow-x-hidden overflow-y-hidden">
          <div className="p-6 border-b border-theme bg-[rgba(8,10,14,0.92)]">
            <h2 className="text-2xl font-bold text-[var(--text-main)]">
              Add Menu Items to Sections
            </h2>
            <p className="text-sm text-[var(--text-main)]/60 mt-1">
              Just click the checkbox to add/remove items instantly!
            </p>
          </div>
          ...
          <div className="px-6 py-4 border-b border-theme">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="text-sm text-[var(--text-main)]/60">
                Showing <span className="text-[var(--text-main)]">{filteredMenuItems.length}</span>{' '}
                of <span className="text-[var(--text-main)]">{menuItems.length}</span> menu items
              </div>
            </div>
          </div>
          ...
          <div className="overflow-hidden">
            <table className="w-full min-w-full">
              <thead className="bg-[rgba(255,255,255,0.03)]">
                <tr>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-[#C59D5F] uppercase tracking-[0.2em]">
                    Menu Item
                  </th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-[#C59D5F] uppercase tracking-[0.2em]">
                    Price
                  ...
                  {sections.map(section => (
                    <th
                      key={section.id}
                      className="px-4 py-4 text-center text-xs font-semibold text-[#C59D5F] uppercase tracking-[0.2em]"
                    >
        ...
        <div className="rounded-xl border border-theme bg-[rgba(8,10,14,0.92)] p-4 mt-8">
          <p className="text-sm text-[var(--text-main)]/60">
            💡 <strong className="font-semibold text-[var(--text-main)]">Tip:</strong> Changes are
            instant and real-time! Check the Order page to see your sections update automatically.
          </p>
        </div>
      </div>
    </m.main>
