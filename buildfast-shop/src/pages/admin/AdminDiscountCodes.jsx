...
function AdminDiscountCodes() {
  ...
  // Theme detection removed - not used in this component
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            if (payload.new && payload.new.id) {
              setCodes(prev =>
                prev.map(code =>
                  code.id === payload.new.id ? payload.new : code
                )
              )
            }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError('Failed to save discount code: ' + (err.message || 'Unknown error'))
    }
  }
  ...
  const handleViewUsage = async (codeId) => {
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-sm text-muted">
                    ...
                    {code.min_order_amount > 0 && (
                      <span>Min: ${parseFloat(code.min_order_amount).toFixed(2)}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2 text-sm text-muted">
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                    ...
                    <div className="rounded-xl border border-theme bg-[rgba(255,255,255,0.03)] p-5">
                      ...
                      <p className="mt-3 text-2xl font-semibold text-emerald-200">
                        ${parseFloat(usageStats.total_revenue || 0).toFixed(2)}
                      </p>
                    </div>
                    <div className="rounded-xl border border-theme bg-[rgba(255,255,255,0.03)] p-5">
                      ...
                      <p className="mt-3 text-2xl font-semibold text-amber-200">
                        ${parseFloat(usageStats.total_discount || 0).toFixed(2)}
                      </p>
                    </div>
                  </div>
                            ...
                            <div className="flex items-start justify-between gap-4">
                              ...
                              <div className="text-right">
                                <p className="text-sm font-semibold text-rose-200">
                                  -${parseFloat(usage.discount_amount || 0).toFixed(2)}
                                </p>
                                <p className="mt-1 text-xs text-muted">
                                  Order: ${parseFloat(usage.order_total || 0).toFixed(2)}
                                </p>
                              </div>
                            </div>
                          </div>
