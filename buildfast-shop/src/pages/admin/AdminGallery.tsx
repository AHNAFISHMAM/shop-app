...
const AdminGallery = () => {
  ...
  const uploadImage = async (file: File, cardId: string, imageType: string): Promise<string | null> => {
    try {
  ...
  // Handle image upload
  const handleImageUpload = async (cardId: string, imageType: 'default' | 'hover', event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (imageUrl) {
      ...
      const { error } = await supabase
        .from('gallery_cards')
        .update({ [updateField]: imageUrl, updated_at: new Date().toISOString() })
        .eq('id', cardId);
  ...
  const updateEffect = async (cardId: string, nextVariants: string[][]) => {
    const baseFallback = Array.isArray(nextVariants) && nextVariants.length > 0 ? nextVariants[0] : ['crossfade'];
  ...
  if (checkingAdmin) {
    return (
      <div className="flex justify-center items-center h-full min-h-[400px]" style={{ backgroundColor: 'var(--bg-main)' }}>
        <div className="text-center space-y-4">
          <div className="animate-spin rounded-full h-16 w-16 border-4 mx-auto" style={{ borderColor: 'var(--accent)', borderTopColor: 'transparent' }}></div>
          <p className="text-sm font-medium" style={{ color: 'var(--accent)' }}>Verifying admin access...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex justify-center items-center h-full min-h-[400px]" style={{ backgroundColor: 'var(--bg-main)' }}>
        <div className="max-w-md mx-auto">
          <div className="glow-surface glow-strong text-center p-8 rounded-2xl border backdrop-blur-xl" style={{ backgroundColor: 'rgba(239, 68, 68, 0.1)', borderColor: 'rgba(239, 68, 68, 0.3)' }}>
            ...
            <h2 className="text-xl font-bold mb-2" style={{ color: '#ef4444' }}>Access Denied</h2>
            <p className="text-sm" style={{ color: 'var(--text-subtitle)' }}>{error || 'Admin privileges required'}</p>
            <button
    ...
    return (
      ...
      >
        <div className="mx-auto max-w-[1600px] px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" role="status" aria-label="Loading gallery cards">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div key={i} className="space-y-4 animate-pulse">
                <div className="aspect-[4/3] bg-[var(--bg-elevated)] rounded-lg" style={{ animationDelay: `${i * 100}ms` }} />
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[1600px] px-6">
        ...
        {/* Gallery Cards List */}
        <section data-animate="fade-scale" data-animate-active="false" className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {galleryCards.map((card, index) => {
            const baseEffects = parseEffects(card.effect);
            const effectSequence = parseEffectVariants(card.effect_variants, baseEffects);
            const captionText = card.caption || card.description || `Card #${card.position} • Gallery showcase`;
            const sequenceSummary = effectSequence
              .map((round, roundIndex) => {
                const labels = round
                  .map((value) => EFFECT_OPTIONS.find((option) => option.value === value)?.label ?? value)
                  .join(' + ');
                if (roundIndex > 0) {
              ...
              >
                ...
                >
                  ...
                  <div className="flex flex-col gap-4 border-b border-theme pb-5 md:flex-row md:items-center md:justify-between">
                    <div>
                      <h3 className="text-lg font-semibold text-text-main">Card #{card.position}</h3>
                      <p className="text-xs text-text-muted uppercase tracking-[0.22em]">Gallery showcase</p>
                    </div>
                    <span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] ${card.is_active ? 'border-emerald-500/40 text-emerald-300' : 'border-rose-500/40 text-rose-300'}`}>
                  ...
                  <div className="flex flex-1 flex-col gap-6 pt-6">
                    ...
                    <div className="flex items-center justify-between rounded-lg border border-theme bg-[rgba(8,10,14,0.92)] px-4 py-3">
                      <div className="flex items-center gap-2 text-xs">
                        <svg className="h-4 w-4 text-[#C59D5F]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <span className="text-xs text-text-muted uppercase tracking-[0.18em]">
                        Updated {formatDistanceToNow(new Date(card.updated_at || card.created_at), { addSuffix: true })}
                      </span>
                    </div>
