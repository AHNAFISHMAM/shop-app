...
function AdminOrders({ fullPage = false }: AdminOrdersProps) {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      setHasMoreOrders(
        filteredData.length === ordersPerPage && resultCount > offset + ordersPerPage
      )
    } catch (err) {
  ...
  const filteredOrders = orders.filter(order => {
    ...
    const matchesPhone =
      normalizedPhone.toLowerCase().includes(normalizedQuery) ||
  ...
  return (
    ...
    >
      <div
        className={`mx-auto ${fullPage ? 'max-w-[1920px] px-4 sm:px-6 lg:px-8' : 'max-w-[1700px] px-4 sm:px-6 md:px-10'}`}
      >
        {/* Header Section */}
        <div
        ...
        >
          <div className="flex-1">
            ...
            <p className="mt-2 max-w-xl text-sm sm:text-base leading-relaxed text-muted">
              Monitor and update every order across the restaurant quickly. Filters update in real
              time with Supabase.
            </p>
          </div>
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (key === 'custom') {
                                ...
                                setDateFilter(
                                  key as 'all' | 'today' | '7days' | '30days' | 'custom'
                                )
                                setStartDate('')
                                setEndDate('')
                                setShowDatePicker(false)
          ...
          (searchQuery || statusFilter !== 'all' || filter !== 'all' || dateFilter !== 'all') && (
            ...
            >
              <p className="text-sm sm:text-base text-muted">
                ...
                of <span className="font-semibold text-[var(--text-main)]">{orders.length}</span>{' '}
        ...
        {error && (
          <div
            className="mb-8 sm:mb-10 rounded-xl sm:rounded-2xl border border-red-500/30 bg-red-500/10 px-4 sm:px-6 md:px-10 py-4 sm:py-5 md:py-6 shadow-[0_18px_45px_-30px_rgba(248,113,113,0.6)]"
            ...
            >
              <div className="w-full overflow-hidden">
                <div className="w-full">
                  <table
                    className="w-full table-fixed divide-y divide-white/10 text-xs sm:text-sm"
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto space-y-8 sm:space-y-10 md:space-y-12 px-4 sm:px-6 md:px-10 py-6 sm:py-8 md:py-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
                  <div>
                    <h3 className="mb-2 text-[10px] sm:text-xs font-semibold uppercase tracking-[0.2em] text-muted">
                      Order Status
                    </h3>
                    <span
                      className={`inline-flex rounded-full px-4 py-3 min-h-[44px] text-sm sm:text-base font-semibold ${getStatusColor(selectedOrder.status)}`}
                    >
                      {String(selectedOrder.status).charAt(0).toUpperCase() +
                        String(selectedOrder.status).slice(1)}
                    </span>
                  </div>
                  <div>
                    <h3 className="mb-2 text-[10px] sm:text-xs font-semibold uppercase tracking-[0.2em] text-muted">
                ...
                <div className="rounded-xl sm:rounded-2xl border border-theme bg-[rgba(255,255,255,0.02)] p-5 sm:p-6 md:p-8">
                  <h3 className="mb-4 sm:mb-5 md:mb-6 text-lg sm:text-xl md:text-2xl font-semibold text-[var(--text-main)]">
                    Customer Information
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
                    <div>
                      ...
                      <p className="mt-1 font-medium text-sm sm:text-base text-[var(--text-main)]">
                    ...
                    <div>
                      ...
                      <span
                        className={`mt-1 inline-flex rounded-full px-3 py-1 text-[10px] sm:text-xs font-semibold ${
                ...
                <div>
                  ...
                  <div className="divide-y divide-white/10 overflow-hidden rounded-xl sm:rounded-2xl border border-theme bg-[rgba(255,255,255,0.02)] text-[var(--text-main)]">
                    {(selectedOrder.order_items || []).map((item: OrderItem, index: number) => (
                      <div
                        key={index}
                        className="flex items-center gap-4 sm:gap-5 md:gap-6 p-5 sm:p-6 md:p-8"
                      >
                        {/* Product Image */}
                  ...
                  <div className="mt-6 sm:mt-8 md:mt-10 rounded-xl sm:rounded-2xl border border-theme bg-[rgba(255,255,255,0.02)] p-5 sm:p-6 md:p-8">
                    <div className="flex items-center justify-between mb-3 sm:mb-4">
                      <span className="text-lg sm:text-xl md:text-2xl font-semibold text-[var(--text-main)]">
                        Order Total
                      </span>
