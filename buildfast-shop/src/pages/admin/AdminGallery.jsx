...
const AdminGallery = () => {
  ...
  const uploadImage = async (file, cardId, imageType) => {
    try {
  ...
  // Handle image upload
  const handleImageUpload = async (cardId, imageType, event) => {
  ...
  // Update card effect
  const updateEffect = async (cardId, nextVariants) => {
  ...
  const toggleActive = async (cardId, currentStatus) => {
    const { error } = await supabase
      .from('gallery_cards')
  ...
  // Show loading gallery cards
  if (loading) {
    return (
      <div className="flex justify-center items-center h-full min-h-[400px]" style={{ backgroundColor: 'var(--bg-main)' }}>
        <div className="text-center space-y-4">
          <div className="animate-spin rounded-full h-16 w-16 border-4 mx-auto" style={{ borderColor: 'var(--accent)', borderTopColor: 'transparent' }}></div>
          <p className="text-sm font-medium" style={{ color: 'var(--accent)' }}>Loading gallery...</p>
        </div>
      </div>
    );
  }
