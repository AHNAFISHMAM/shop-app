...
function AdminReservations() {
  ...
  const getConflicts = (reservation: Reservation) => {
    ...
    return reservations.filter(r => {
      ...
      if (r.status === 'cancelled' || r.status === 'declined' || r.status === 'no_show')
        return false
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider ${styles[status] || 'bg-gray-500/20 text-[var(--text-muted)] border border-gray-500/30'}`}
      >
      ...
      <div className="mx-auto max-w-[1600px] px-4 sm:px-6 md:px-10 py-3 sm:py-4 md:py-5">
        ...
        >
          <div className="grid gap-3 sm:gap-4 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              <label className="mb-2 block text-sm sm:text-base font-medium text-muted">
                Status
              </label>
              <CustomDropdown
                options={[
        ...
        {/* Waitlist Section */}
        <div className="mb-6" data-animate="fade-scale" data-animate-active="false">
          <WaitlistManager />
        </div>
                    ...
                    <div className="mb-4 flex items-center justify-between border-b border-theme pb-3">
                      <div>
                        ...
                        <p className="text-xs text-muted">
                          {groupedReservations[date]?.length || 0} reservations
                        </p>
                      </div>
                      <div className="rounded-lg bg-[var(--accent)]/20 px-3 py-1.5">
                        <span className="text-sm font-bold text-[var(--accent)]">
                          {groupedReservations[date]?.length || 0}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead className="border-b border-theme bg-[rgba(255,255,255,0.03)]">
                        <tr>
                          <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-muted">
                            Time
                          </th>
                          <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-muted">
                            Customer
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="mt-1 text-sm text-orange-200/80">
                            {getConflicts(selectedReservation).length} other reservation(s) at the
                            same time:
                          </p>
                          <ul className="mt-2 space-y-1 text-sm">
                            {getConflicts(selectedReservation).map(conflict => (
                              <li key={conflict.id} className="text-orange-200/70">
                                • {conflict.customer_name} - {conflict.party_size} guests{' '}
                                {conflict.table_number && `(Table ${conflict.table_number})`}
                              </li>
                            ))}
                          </ul>
                        </div>
                  ...
                  <div className="mb-6 space-y-4">
                    ...
                    {selectedReservation.special_requests && (
                      <div>
                        ...
                        <p className="mt-1 rounded-lg bg-[rgba(255,255,255,0.05)] p-3 text-sm text-[var(--text-main)]">
                          {String(selectedReservation.special_requests || '')}
                        </p>
                      </div>
                    )}

                    {selectedReservation.admin_notes &&
                      selectedReservation.admin_notes !== adminNotes && (
                        <div>
                          <p className="text-sm font-medium text-muted">Previous Admin Notes</p>
                          <p className="mt-1 rounded-lg bg-[rgba(255,255,255,0.05)] p-3 text-sm text-muted">
                            {selectedReservation.admin_notes}
                          </p>
                        </div>
                      )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 md:gap-6">
                    {selectedReservation.status === 'pending' && (
                      <>
                        ...
                        <button
                          onClick={() =>
                            updateReservationStatus(selectedReservation.id, 'declined', adminNotes)
                          }
                    ...
                    {selectedReservation.status === 'confirmed' && (
                      <>
                        ...
                        <button
                          onClick={() =>
                            updateReservationStatus(selectedReservation.id, 'no_show', adminNotes)
                          }
        ...
        >
          ...
          >
            ...
            <p className="mb-6 text-sm text-muted">
              Are you sure you want to permanently delete this reservation? All associated data will
      ...
      {showSettings && (
        ...
        >
          ...
          >
            ...
            <div className="sticky top-0 flex items-center justify-between border-b border-theme bg-[rgba(5,5,9,0.95)] p-6 backdrop-blur-sm">
              <div>
                ...
                <p className="text-sm text-muted mt-1">
                  Configure how customers can make reservations
                </p>
              </div>
              <button
                onClick={() => setShowSettings(false)}
            ...
            <div
              data-overlay-scroll
              className="p-6 max-h-[calc(100vh-12rem)] overflow-y-auto hide-scrollbar"
            >
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-theme bg-[rgba(5,5,9,0.95)] p-4 backdrop-blur-sm flex-shrink-0">
                <div>
                  <h2
                    ...
                    className="text-lg font-bold text-[var(--text-main)] flex items-center gap-2"
                  >
                  ...
                  <p className="text-xs text-muted mt-0.5">
                    Select a date to view reservation details
                  </p>
                </div>
                <button
                  onClick={() => setShowCalendarModal(false)}
              ...
              <div className="flex-1 flex flex-col lg:flex-row overflow-hidden min-h-0">
                ...
                <div className="flex-1 lg:flex-[0_0_50%] p-4 sm:p-6 overflow-y-auto hide-scrollbar border-r-0 lg:border-r border-theme flex items-center justify-center">
                  <Calendar value={selectedCalendarDate} onChange={setSelectedCalendarDate} />
                </div>
                        ...
                        {dayReservations.length > 0 ? (
                          ...
                          <div className="flex-1 flex items-center justify-center">
                            <div className="text-center">
                              <svg
                                className="h-16 w-16 mx-auto mb-4 text-muted opacity-50"
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                              >
