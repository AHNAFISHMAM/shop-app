import { useCallback, useEffect, useMemo, useState, ChangeEvent, MouseEvent } from 'react'
...
const resolveStatus = (customer: Customer): Customer['status'] => {
...
const AdminCustomers = (): JSX.Element => {
  ...
  const segmentAnalytics = useMemo((): SegmentAnalytic[] => {
    if (customers.length === 0) {
      return [
  ...
  const handleToggleBlacklist = async (customer: Customer): Promise<void> => {
    if (!customer?.id) {
  ...
  return (
    ...
    >
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to flag "${customerToBlacklist?.name || customerToBlacklist?.email}" for review and restrict activity?\n\nThis will prevent them from making orders or reservations.`}
