  ...
  const confirmAddAdmin = async () => {
    ...
    try {
      ...
      if (rpcData?.success) {
        ...
        setError(rpcData?.error || 'Failed to add admin')
      }
    } catch (err) {
  ...
  const confirmRemoveAdmin = async () => {
    ...
    try {
      ...
      if (rpcData?.success) {
        ...
        setError(rpcData?.error || 'Failed to remove admin')
      }
    } catch (err) {
      ...
      <div className="mx-auto max-w-[1400px] px-4 sm:px-6 md:px-10 py-3 sm:py-4 md:py-5">
        ...
        <div className="mb-8 bg-theme-elevated rounded-xl sm:rounded-2xl shadow-sm p-4 sm:p-6 md:p-10 border border-theme">
          ...
          <p className="text-sm text-[var(--text-muted)] mb-6">
            Enter the email address of the user you want to grant admin privileges to. The user must
              ...
              {admins.map(admin => (
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-[var(--text-muted)] mt-1">{admin.email}</div>
