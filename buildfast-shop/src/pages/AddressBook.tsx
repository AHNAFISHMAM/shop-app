...
const AddressBook = memo((): JSX.Element | null => {
  ...
  const handleEdit = useCallback((address: Address): void => {
    setEditingAddress(address)
    setShowModal(true)
  }, [])

  const handleSave = useCallback(async (formData: Partial<Address>): Promise<void> => {
    if (!user) return
  ...
  const openDeleteConfirm = useCallback((address: Address): void => {
  ...
  const handleSetDefault = useCallback(async (address: Address): Promise<void> => {
    if (!user) return
    ...
    >
      ...
      <div className="app-container px-4 sm:px-6 md:px-10 py-3 sm:py-4 md:py-5">
          ...
          <div className="glow-surface glow-soft mb-8 overflow-hidden rounded-xl sm:rounded-2xl border border-theme bg-[var(--bg-main)] px-4 sm:px-6 md:px-10 py-3 sm:py-4 md:py-5">
            ...
            <div className="relative z-10 flex flex-col gap-3 sm:gap-4 md:gap-6 md:flex-row md:items-center md:justify-between">
              <div className="space-y-2 max-w-xl">
                <p className="text-sm sm:text-xs uppercase tracking-[0.3em] text-[var(--color-amber)]/70">Star Rewards</p>
                ...
                <div className="flex items-center gap-3 sm:gap-4 md:gap-6 pt-2">
                  <div className="relative h-2 w-full max-w-xs rounded-full bg-white/10 overflow-hidden" role="progressbar" aria-valuenow={loyalty.progressPercent} aria-valuemin={0} aria-valuemax={100} aria-label="Loyalty progress">
                    <div
              ...
              {referral && (
                <div className="flex w-full max-w-xs flex-col gap-3 sm:gap-4 md:gap-6 rounded-xl sm:rounded-2xl border border-theme bg-theme-elevated px-4 sm:px-6 md:px-10 py-3 sm:py-4 md:py-5 backdrop-blur-sm">
                  <div>
                    ...
                    <p className="mt-1 text-sm sm:text-xs text-[var(--text-main)]/70">{referral.subcopy}</p>
                  </div>
        ...
        {success && (
          ...
          >
            <div className="flex items-center gap-3 sm:gap-4 md:gap-6">
              <svg className="w-5 h-5 text-[var(--color-emerald)]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <p className="text-sm sm:text-base font-medium text-[var(--color-emerald)]">{success}</p>
            </div>
          </div>
        )}
        ...
        {error && (
          ...
          >
            <div className="flex items-center gap-3 sm:gap-4 md:gap-6">
              <svg className="w-5 h-5 text-[var(--color-red)]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        {/* Loading State */}
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6" role="status" aria-live="polite" aria-busy="true">
            {[1, 2, 3].map((i) => (
          ...
          /* Address Cards Grid */
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6" role="list" aria-label="Saved addresses">
