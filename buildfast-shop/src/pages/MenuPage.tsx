...
const MenuPage = memo(() => {
  ...
  useEffect(() => {
    const initialCount =
      totalFilteredCount > 0 ? Math.min(ITEMS_PER_BATCH, totalFilteredCount) : ITEMS_PER_BATCH
  ...
  if (loading) {
    return (
      <MenuPageSkeleton isLightTheme={isLightTheme} prefersReducedMotion={prefersReducedMotion} />
    )
  }
    ...
    <>
      ...
      >
        ...
        <m.div
          ...
          exit="exit"
        >
          <MenuSearchBar searchQuery={searchQuery} onSearchChange={handleSearchChange} />
        </m.div>
        ...
        >
          <m.div className="space-y-1" variants={fadeSlideUp}>
            <p className="text-sm uppercase tracking-[0.25em] sm:tracking-[0.3em] text-[var(--text-secondary)]">
              Skip the queue
            </p>
            <p
              id="reservation-cta-heading"
              className="text-sm text-[var(--text-main)]/80 leading-relaxed"
            >
              Book a table and stage your cart before you arrive.
            </p>
          </m.div>
          <m.div className="flex flex-wrap gap-2 sm:gap-3" variants={fadeSlideUp}>
            <Link
              to="/reservations"
        ...
        >
          ...
          >
            ...
            {selectedCategory && (
              <span
                className="ml-auto text-sm px-2 py-1 rounded-full bg-[var(--accent)]/20 text-[var(--accent)]"
                aria-label={`Selected category: ${selectedCategory.name}`}
              >
        ...
        >
          ...
          <CollapsibleSidebar
            ...
            menuItems={menuItems.filter((item) => item.category_id !== null).map(item => ({ ...item, category_id: item.category_id! })) as any}
