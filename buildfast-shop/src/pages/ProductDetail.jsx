...
} from '../lib/variantCombinationsUtils'
...
function ProductDetail() {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      // Single-variant product: use variant
        ...
        variantParam = getSelectedVariant()
      }
  ...
  const handleReviewSuccess = (data, warning) => {
    setShowReviewForm(false)
  ...
  if (error || !product) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--bg-main)] px-4 text-[var(--text-main)]">
        <div className="glow-surface glow-strong w-full max-w-md rounded-2xl border border-red-400/40 bg-red-500/10 p-8 text-center">
          ...
          <p className="mb-6 text-sm text-red-100">{error || 'The product you are looking for does not exist.'}</p>
          <Link
            to="/products"
  ...
  const showStockCount = typeof currentStock === 'number' && currentStock !== null
  const hasVariants = Object.keys(variants).length > 0
    ...
    >
      ...
      >
        <div className="app-container py-3 sm:py-4">
          <nav className="flex items-center space-x-2 text-[10px] sm:text-xs uppercase tracking-[0.25em] text-muted">
            ...
            <Link to="/products" className="transition hover:text-[var(--accent)]">
              Products
            </Link>
            <svg className="h-3 w-3 text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
      ...
      <m.section
        ...
        custom={0.18}
      >
        <div className="glow-surface glow-strong overflow-hidden rounded-xl sm:rounded-2xl border border-theme bg-[rgba(255,255,255,0.02)] shadow-[0_35px_90px_-55px_rgba(197,157,95,0.55)]">
          <div className="grid grid-cols-1 gap-6 sm:gap-8 p-4 sm:p-6 md:p-10 lg:grid-cols-2">
            {/* Image Gallery */}
            <div className="space-y-3 sm:space-y-4">
              {/* Main Image */}
              <div className="relative aspect-square overflow-hidden rounded-xl sm:rounded-2xl bg-[var(--bg-main)]/20">
                <img
                ...
                />
                {isOutOfStock && (
                  <div className="absolute right-3 sm:right-4 top-3 sm:top-4 rounded-lg bg-red-500/90 px-3 sm:px-4 py-2 text-[10px] sm:text-xs font-semibold text-black shadow-lg">
                    Out of Stock
                  </div>
                )}
              </div>
            ...
            <div className="flex flex-col">
              ...
              {success && successMessage && (
                <div className="glow-surface glow-soft mb-4 sm:mb-6 rounded-xl sm:rounded-2xl border border-emerald-400/30 bg-emerald-500/10 p-3 sm:p-4">
                  <div className="flex items-center gap-3 text-[10px] sm:text-xs text-emerald-200">
              ...
              {error && (
                <div className="glow-surface glow-soft mb-4 sm:mb-6 rounded-xl sm:rounded-2xl border border-red-400/40 bg-red-500/15 p-3 sm:p-4">
                  <div className="flex items-center gap-3 text-[10px] sm:text-xs text-red-200">
                    <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {product.category && (
                <div className="mb-3 sm:mb-4">
                  <span className="inline-block rounded-full bg-[var(--accent)]/15 px-3 py-1 text-[10px] sm:text-xs font-semibold text-[var(--accent)]">
                    {product.category}
                  </span>
                </div>
              )}
              ...
              {/* Product Name */}
              <h1 className="mb-3 sm:mb-4 text-xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold">
                {product.name}
              </h1>
              ...
              {hasVariants && !loadingVariants && (
                <div className="mb-4 sm:mb-6 space-y-3 sm:space-y-4">
                  {Object.keys(variants).map((variantType) => {
                    ...
                    const selectedVariant = selectedVariants[variantType]
                          ...
                          {variantOptions.map((variant) => {
                            ...
                            return (
                              <button
                                ...
                                className={`rounded-xl sm:rounded-2xl px-4 py-3 min-h-[44px] text-sm sm:text-base font-medium transition-all ${
                                  isSelected
                                    ? 'border border-[var(--accent)]/70 bg-[var(--accent)]/90 text-[#111] shadow-[0_12px_30px_-20px_rgba(197,157,95,0.6)]'
                                    : isOutOfStockVariant
                                    ? 'cursor-not-allowed border border-theme bg-white/10 text-muted line-through'
                                    : 'border border-theme bg-white/5 text-[var(--text-main)] hover:border-[var(--accent)]/60 hover:text-[var(--accent)]'
                                }`}
                                title={isOutOfStockVariant ? 'Out of stock' : variant.variant_value}
                              >
                                {variant.variant_value}
                                {variant.price_adjustment !== 0 && (
                                  <span className="ml-1 text-[10px] sm:text-xs">
                                    ({variant.price_adjustment > 0 ? '+' : ''}{getCurrencySymbol(product?.currency)}{formatPrice(Math.abs(variant.price_adjustment), 0)})
                                  </span>
                                )}
                              </button>
                            )
                ...
                <button
                  ...
                  className={`flex w-full items-center justify-center gap-2 rounded-xl sm:rounded-2xl px-6 py-3 min-h-[44px] text-sm sm:text-base font-semibold transition ${
                    isOutOfStock || addingToCart
                      ...
                      : 'bg-[var(--accent)] text-[#111] shadow-[0_18px_45px_-30px_rgba(197,157,95,0.7)] hover:opacity-90'
                  }`}
                >
                  {addingToCart ? (
                    <>
                    ...
                    <>
                      <svg
                        className={`w-5 h-5 transition-colors ${
                          isProductInFavorites ? 'fill-current text-red-200' : 'fill-none'
                        }`}
                        stroke="currentColor"
                        strokeWidth={2}
                        viewBox="0 0 24 24"
